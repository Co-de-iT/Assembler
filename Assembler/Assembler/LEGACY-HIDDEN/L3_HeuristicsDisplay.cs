...
{
    [Obsolete]
    public class L3_HeuristicsDisplay : GH_CustomPreviewComponent
    {
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            for (int i = 0; i < _edgeCatalog.Length; i++)
                _edgeCatalog[i] = MeshUtils.GetSilhouette(components[i].CollisionMesh);
        ...
        private DataTree<AssemblyObject> GeneratePairs(AssemblyObject[] AO, List<Rule> Hr, Point3d O, double padX, double padY, int nR)
        {
            ...
            for (int i = 0; i < Hr.Count; i++)
            {
                ...
                Transform orientStoR = Transform.PlaneToPlane(senderAO.Handles[sH].SenderPlane, receiverAO.Handles[rH].ReceiverPlanes[rR]);
